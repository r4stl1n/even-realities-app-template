        ...
        if centralManager == nil {
            centralManager = CBCentralManager(
                ...
                options: [
                    ...
                    CBCentralManagerOptionRestoreIdentifierKey: "com.eventemplate.central.r1",
                ]
            )
        }
...
extension R1: CBCentralManagerDelegate {
    ...
    /// restoration). Adopt the ring so the session continues; the MAC needed
    /// for the glasses link handshake was persisted at original pairing.
    nonisolated func centralManager(_: CBCentralManager, willRestoreState dict: [String: Any]) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            Bridge.log("R1: Restoring peripheral: \(peripheral.name ?? "ring") (state: \(peripheral.state.rawValue))")
            self.ringPeripheral = peripheral
            peripheral.delegate = self
            if peripheral.state == .connected {
                peripheral.discoverServices(nil)
            }
            // .connecting / .disconnected rings are picked up by the normal
            // connectByUUID path once the central reports poweredOn.
        }
    }
