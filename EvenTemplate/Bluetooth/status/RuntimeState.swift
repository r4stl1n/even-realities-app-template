...
public enum GlassesRuntimeState: CustomStringConvertible {
    ...
    public var connection: GlassesConnectionState {
        ...
        case let .disconnected(connection):
            connection
        case let .connected(_, connection, _, _, _, _, _):
            connection
        }
    }
    ...
    public var ready: Bool {
        ...
        case .disconnected:
            false
        case let .connected(_, _, _, _, ready, _, _):
            ready
        }
    }

    public var battery: GlassesBatteryState? {
        guard case let .connected(battery, _, _, _, _, _, _) = self else {
            return nil
        }
        return battery
    }
    ...
    public var device: ConnectedGlassesInfo? {
        guard case let .connected(_, _, device, _, _, _, _) = self else {
            return nil
        }
        return device
    }
    ...
    public var firmware: FirmwareInfo? {
        guard case let .connected(_, _, _, firmware, _, _, _) = self else {
            return nil
        }
        return firmware
    }
    ...
    public var signal: SignalState? {
        guard case let .connected(_, _, _, _, _, signal, _) = self else {
            return nil
        }
        return signal
    }
    ...
    public var voiceActivityDetectionEnabled: Bool {
        guard case let .connected(_, _, _, _, _, _, vad) = self else {
            return false
        }
        return vad
    }
    ...
    public var description: String {
        ...
        case let .connected(_, _, device, _, ready, _, _):
