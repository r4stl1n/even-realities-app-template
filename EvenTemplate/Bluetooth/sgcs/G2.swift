...
private enum ServiceID: UInt8 {
    ...
    case moduleConfigure = 32  // 0x20 - SERVICE_MODULE_CONFIGURE_APP_ID (dashboard auto-close, language)
...
// MARK: - Module Configure Protobuf Builders (module_configure.proto, service ID 32)

/// Builders for the module_configure service (0x20). This is where the
/// dashboard's auto-close timer lives — how long the dashboard stays on screen
/// before the firmware closes it and blanks the lens.
///
/// Schema (from the module_configure.proto descriptor):
///   module_configure_main_msg_ctx {
///     cmd = 1 (module_configure_Cmd_list)
///     magicRandom = 2
///     SysGeneralSetting = 3 { languageIndex = 1 }
///     DashboardGeneralSetting = 4 { autoCloseValue = 1 }
///   }
private enum ModuleConfigureProto {
    /// module_configure_Cmd_list
    enum CommandId: Int32 {
        case systemGeneralSettingPacket = 0
        case inquireDashboardAutoClose = 1  // APP_INQUIRE_DASHBOARD_AUTO_CLOSE_VALUE
        case setDashboardAutoClose = 2  // APP_SET_DASHBOARD_AUTO_CLOSE_VALUE
    }
    ...
    static func setDashboardAutoClose(magicRandom: Int32, seconds: Int32) -> Data {
        // dashboard_general_setting
        var dashW = ProtobufWriter()
        dashW.writeInt32Field(1, seconds)  // autoCloseValue

        // module_configure_main_msg_ctx
        var w = ProtobufWriter()
        w.writeInt32Field(1, CommandId.setDashboardAutoClose.rawValue)  // cmd
        w.writeInt32Field(2, magicRandom)  // magicRandom
        w.writeMessageField(4, dashW.data)  // DashboardGeneralSetting (field 4)
        return w.data
    }

    /// APP_INQUIRE_DASHBOARD_AUTO_CLOSE_VALUE — ask the glasses for the current value.
    static func inquireDashboardAutoClose(magicRandom: Int32) -> Data {
        var w = ProtobufWriter()
        w.writeInt32Field(1, CommandId.inquireDashboardAutoClose.rawValue)  // cmd
        w.writeInt32Field(2, magicRandom)  // magicRandom
        return w.data
    }
}
...
class G2: NSObject, SGCManager {
    ...
    private func runAuthSequence() async {
        ...
        // Apply the stored "Hey Even" wakeword setting on connect (off by default)
        let heyEvenEnabled =
            DeviceStore.shared.get("bluetooth", "hey_even_enabled") as? Bool ?? false
        let heyEvenMsg = EvenAIProto.setHeyEven(
            magicRandom: self.sendManager.nextMagicRandom(),
            enabled: heyEvenEnabled
        )
        self.sendEvenAICommand(heyEvenMsg)
        Bridge.log("G2: Set Hey Even wakeword \(heyEvenEnabled ? "ON" : "OFF")")

        // Dashboard auto-close: apply ours if set, otherwise read theirs so the
        // UI can show the glasses' current value.
        let autoClose =
            DeviceStore.shared.get("bluetooth", "dashboard_auto_close_seconds") as? Int ?? -1
        if autoClose >= 0 {
            self.setDashboardAutoCloseSeconds(autoClose)
        } else {
        ...
        // replace the firmware default menu with ours (Notification + stored apps)
        self.sendMenuApps()
    ...
    private func sendMenuApps() {
        ...
        let hideBuiltins =
            DeviceStore.shared.get("bluetooth", "hide_builtin_menu_apps") as? Bool ?? true
        // With nothing to add and hiding disabled, leave the firmware default
        // menu (Even AI, Teleprompt, Translate, Navigate, Conversate) alone.
        if menuItems.isEmpty && !hideBuiltins {
            return
        }
        // Otherwise send even when empty: APP_SEND_MENU_INFO replaces the
        // firmware default menu, leaving only the built-in Notification entry
        // plus placeholder slots.
    ...
    /// Approximate width of one glyph at the firmware's default text size, in
    /// canvas px. The text container proto exposes no alignment or font-metric
    /// fields, so horizontal centering is an estimate — nudge this constant if
    /// short strings sit visibly off-center.
    private static let approxGlyphWidth: Int32 = 20

    /// Center a short single-line string on the canvas. Vertical centering is
    /// exact (one firmware line in a 288px canvas); horizontal centering uses
    /// the glyph-width estimate above, with slack so an under-estimate can't
    /// wrap the text onto a second line and break the vertical centering.
    func sendCenteredText(_ text: String) async {
        let canvasWidth = G2.defaultTextContainer.width
        let canvasHeight = G2.defaultTextContainer.height
        let lineHeight = G2.minTextContainerHeight
        let padding = G2.defaultTextContainer.paddingLength

        let estimated = (Int32(text.count) + 2) * G2.approxGlyphWidth  // +2 glyphs of slack
        let boxWidth = min(estimated + 2 * padding, canvasWidth)
        let x = max((canvasWidth - boxWidth) / 2, 0)
        let y = max((canvasHeight - lineHeight) / 2, 0)

        await sendTextAt(text, x: x, y: y, width: boxWidth, height: lineHeight)
    }
    ...
    /// lands in [handleModuleConfigureResponse].
    func requestDashboardAutoClose() {
        let msg = ModuleConfigureProto.inquireDashboardAutoClose(
            magicRandom: sendManager.nextMagicRandom()
        )
        sendModuleConfigureCommand(msg)
    }
    ...
    func setHeyEvenEnabled(_ enabled: Bool) {
        ...
        Bridge.log("G2: Set Hey Even wakeword \(enabled ? "ON" : "OFF")")
    }
        ...
        if centralManager == nil {
            centralManager = CBCentralManager(
                ...
                options: [
                    ...
                    CBCentralManagerOptionRestoreIdentifierKey: "com.eventemplate.central.g2",
                ]
            )
        }
    ...
    private func parseDeviceRequestResponse(_ data: Data) {
        ...
        logUnknownSettingFields(fields, known: [5, 6, 12, 13], source: "DeviceRequestResponse")
    }

    /// Discovery hook for settings we haven't reverse-engineered yet (e.g. a
    /// display-sleep / screen-timeout value). Every setting in this file was
    /// derived by diffing these dumps while toggling the setting in the official
    /// Even app — an unrecognised field appearing here is the field to implement.
    /// Known fields are filtered out so this stays quiet during normal polling.
    private func logUnknownSettingFields(
        _ fields: [Int: Any], known: Set<Int>, source: String
    ) {
        let unknown = fields.keys.filter { !known.contains($0) }.sorted()
        guard !unknown.isEmpty else { return }
        let described = unknown.map { key -> String in
            if let intValue = fields[key] as? Int32 { return "\(key)=\(intValue)" }
            if let data = fields[key] as? Data {
                return "\(key)=0x\(data.map { String(format: "%02X", $0) }.joined())"
            }
            return "\(key)=?"
        }.joined(separator: " ")
        Bridge.log("G2: \(source) unknown fields: \(described)")
    }

    /// module_configure service (0x20). Carries the dashboard auto-close value,
    /// both as an echo of our set and as the reply to an inquire.
    private func handleModuleConfigureResponse(_ payload: Data) {
        var reader = ProtobufReader(payload)
        let fields = reader.parseFields()
        let cmd = fields[1] as? Int32 ?? -1

        if let dashData = fields[4] as? Data {
            var dashReader = ProtobufReader(dashData)
            let dashFields = dashReader.parseFields()
            if let value = dashFields[1] as? Int32 {
                let readable =
                    value == ModuleConfigureProto.autoCloseDisabled ? "never" : "\(value)s"
                Bridge.log("G2: dashboard auto-close = \(readable) (cmd=\(cmd))")
                DeviceStore.shared.set("glasses", "dashboardAutoCloseSeconds", Int(value))
                return
            }
        }
    ...
    private func parseDeviceSendToApp(_ data: Data) {
        ...
        logUnknownSettingFields(fields, known: [1, 2], source: "DeviceSendToApp")
    }
...
extension G2: CBCentralManagerDelegate {
    ...
    /// Called when iOS relaunches the app in the background to hand back
    /// connections it kept alive after the app was terminated (state
    /// restoration). Adopt the peripherals so the session continues.
    nonisolated func centralManager(_: CBCentralManager, willRestoreState dict: [String: Any]) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            for peripheral in peripherals {
                ...
                Bridge.log("G2: Restoring peripheral: \(name) (state: \(peripheral.state.rawValue))")
                peripheral.delegate = self
                if name.contains("_L_") {
                    self.leftPeripheral = peripheral
                } else if name.contains("_R_") {
                ...
                // .connecting / .disconnected peripherals are picked up by the
                // normal reconnect path once the central reports poweredOn.
            }
        }
    }
