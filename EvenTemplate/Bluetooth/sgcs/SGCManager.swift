...
protocol SGCManager {
    ...
    /// Render a short single-line message in the middle of the display.
    /// Default implementation falls back to [sendTextWall] (top-left); G2
    /// overrides to position the text at the center of its canvas.
    ...
    // MARK: - Hey Even Wakeword

    /// Enable/disable the "Hey Even" voice wakeword on the glasses.
    /// Default no-op; only G2 supports this today.
    func setHeyEvenEnabled(_ enabled: Bool)

    // MARK: - Dashboard Auto-Close

    /// How long the dashboard stays up before the firmware closes it and blanks
    /// the lens. `seconds <= 0` means never. Default no-op; only G2 supports this.
...
extension SGCManager {
    ...
    // MARK: - Centered Text (default: top-left text wall — only G2 can position)

    func sendCenteredText(_ text: String) async {
        await sendTextWall(text)
    }

    // MARK: - Hey Even Wakeword (default no-op — only G2 supports this)

    func setHeyEvenEnabled(_: Bool) {
        Bridge.log("SGC: setHeyEvenEnabled not supported")
    }

    // MARK: - Dashboard Auto-Close (default no-op — only G2 supports this)

    func setDashboardAutoCloseSeconds(_: Int) {
        Bridge.log("SGC: setDashboardAutoCloseSeconds not supported")
    }
