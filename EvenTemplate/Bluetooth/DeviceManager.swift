...
class DeviceManager: NSObject {
    ...
    func handleDeviceReady() {
        ...
        // Splash screen on first connect for all display glasses: hold the
        // template banner briefly, then clear it and hand the display back to
        // the glasses dashboard.
        let splashSeconds =
            DeviceStore.shared.get("bluetooth", "splash_duration_seconds") as? Int ?? 2
        if shouldSendBootingMessage && splashSeconds > 0 {
            Task {
                let splash = "-- template --"
                await sgc.sendCenteredText(splash)
                try? await Task.sleep(nanoseconds: UInt64(splashSeconds) * 1_000_000_000)

                // Wipe the splash by overwriting the same box with spaces. The
                // identical character count keeps the computed rect identical, so
                // this reuses the existing container and repaints it empty via the
                // ordinary text-update path — what actually clears pixels.
                // clearDisplay() alone can't be relied on here: a positioned box is
                // not full-canvas, so it takes the structural-purge branch and
                // repaints via an async page rebuild.
                await sgc.sendCenteredText(String(repeating: " ", count: splash.count))
                try? await Task.sleep(nanoseconds: 400_000_000)

                // Now drop the container and hand the display to the dashboard.
                // Ordered after the blank so the rebuild clearDisplay() schedules
                // can't race the page teardown inside showDashboard().
                sgc.clearDisplay()
